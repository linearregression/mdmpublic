...
## --
## Created : <2015-05-28>
## Updated: Time-stamp: <2015-06-14 22:29:07>
##-------------------------------------------------------------------
function log() {
    local msg=${1?}
...
mkdir -p /root/docker/ && chmod 777 -R /root/docker/
...
log "Avoid unnecessary Jenkins restart"
container_id=$(docker ps | grep mdm-jenkins | awk -F' ' '{print $1}')
docker exec mdm-jenkins sed -i "s/generated by Chef for.*/generated by Chef for $container_id./g" /etc/default/jenkins
rm -rf /root/docker/*
